...
export class PlanRegistry {
    ...
    for (const [key, type] of Object.entries(incoming.types)) {
      ...
      if (owner && owner !== partId && partId) {
        t.error(
          ...
          { key, owner, incoming: partId },
          new Error(`Cross-source type collision on key "${key}": owned by ${owner}, incoming from ${partId}`),
        );
        continue;
      }
  ...
  private mergeComponents(incoming: Plan, target: Plan, partId: string | undefined): void {
    for (const [key, comp] of Object.entries(incoming.components)) {
      ...
      if (owner && owner !== partId && partId) {
        t.error(
          "merge.component.collision",
          { key, owner, incoming: partId },
          new Error(`Cross-source component collision on key "${key}": owned by ${owner}, incoming from ${partId}`),
        );
        continue;
      }
