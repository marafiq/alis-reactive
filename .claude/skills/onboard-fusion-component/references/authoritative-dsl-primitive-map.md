...
| An **opaque vendor callback** passed as a method arg/property value | `ValueExpression` has no function node — kinds are `literal/read/object/array/array-op` only; a JS function cannot be serialized. **NOT a wall when the callback is:** an element **predicate** → `ConditionGraph` via array-op `Where/Any/All/Find`; an element **projection/accessor** → `ValueExpression` via array-op `Select/Map`; a **formatter/template** → `FusionTemplate.Create<T>`. See "Not a wall" below + the array sandbox. Only a callback that is none of these is the wall | `ValueExpression.cs:19-163` · `ReactiveArray.cs` | → element predicate/projection: EXPRESS IT (not a wall); else builder-owned template/config or plugin · recorded structurally |
