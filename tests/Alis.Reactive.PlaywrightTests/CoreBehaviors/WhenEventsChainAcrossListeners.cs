...
public class WhenEventsChainAcrossListeners : PlaywrightTestBase
{
    ...
    {
        // Workflow 3 dispatches "final" with payload { eventName: "done" }.
        ...
        var finalDispatch = _consoleMessages
            .FirstOrDefault(m => m.Contains("\"span_name\":\"alis.action.dispatch\"")
                                 && m.Contains("\"alis.event_name\":\"final\""));

        Assert.That(finalDispatch, Is.Not.Null,
            "final dispatch must be traced");
        Assert.That(_consoleMessages.Any(m =>
            m.Contains("\"event_name\":\"dispatch.payload\"")
            && m.Contains("\"alis.payload.eventName\":\"done\"")), Is.True,
            "dispatch payload must survive the chain and be visible in trace events");
    ...
    {
        // Workflow 4 does RemoveClass("text-text-muted") then AddClass("text-green-600").
    ...
    public async Task dispatches_occur_in_chronological_order_in_trace()
    {
        ...
        var dispatches = _consoleMessages
            .Where(m => m.Contains("\"span_name\":\"alis.action.dispatch\""))
            .ToList();

        var testIdx = dispatches.FindIndex(m =>
            m.Contains("\"alis.event_name\":\"test\"") && !m.Contains("test-received"));
        var receivedIdx = dispatches.FindIndex(m =>
            m.Contains("\"alis.event_name\":\"test-received\""));
    ...
    public async Task all_three_steps_have_green_class_after_chain_completes()
    {
        ...
        // or workflow abort), that step's element will NOT have text-green-600.
    ...
    {
        // Workflow 4 (on "final") calls RemoveClass("text-text-muted") on #chain-status
    ...
    {
        // The chain-status element receives a 3-mutation sequence from Workflow 4 (on "final"):
