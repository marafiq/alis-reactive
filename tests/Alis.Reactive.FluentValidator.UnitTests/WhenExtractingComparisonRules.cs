...
public class WhenExtractingComparisonRules
{
    ...
    {
        var desc = _ruleExtractor.ExtractRules(typeof(MinComparisonValidator), "testForm");

        Assert.That(desc, Is.Not.Null);
        Assert.That(desc!.Fields[0].Rules[0].Rule, Is.EqualTo("min"));
        Assert.That(desc.Fields[0].Rules[0].Constraint, Is.EqualTo(0m));
        Assert.That(desc.Fields[0].Rules[0].ShapeToken, Is.EqualTo("number"));
    }

    [Test]
    public void LessThanOrEqualTo_produces_max_rule_with_number_coercion()
    {
        var desc = _ruleExtractor.ExtractRules(typeof(MaxComparisonValidator), "testForm");
    ...
    {
        var desc = _ruleExtractor.ExtractRules(typeof(StrictGreaterThanValidator), "testForm");
    ...
    {
        var desc = _ruleExtractor.ExtractRules(typeof(StrictLessThanValidator), "testForm");
