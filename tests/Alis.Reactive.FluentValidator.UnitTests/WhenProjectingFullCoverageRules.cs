...
public class WhenProjectingFullCoverageRules
{
    ...
    public void Name_minLength_projects_with_constraint()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(3));
    }
    ...
    public void Name_maxLength_projects_with_constraint()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(100));
    }
    ...
    public void Phone_projects_regex_with_pattern()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(@"^\d{3}-\d{3}-\d{4}$"));
    }
    ...
    public void Score_exclusiveRange_projects_with_number_coercion()
    {
        ...
        var range = rule.RangeOperand();
        Assert.That(range.LowerBound, Is.EqualTo(0m));
        Assert.That(range.UpperBound, Is.EqualTo(100m));
        Assert.That(rule.Shape, Is.EqualTo(Alis.Reactive.PlanModel.Shape.Number));
    }
    ...
    public void Salary_min_projects_with_number_coercion()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(0m));
        Assert.That(rule.Shape, Is.EqualTo(Alis.Reactive.PlanModel.Shape.Number));
    }
    ...
    public void Salary_max_projects_with_number_coercion()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(500000m));
        Assert.That(rule.Shape, Is.EqualTo(Alis.Reactive.PlanModel.Shape.Number));
    }
    ...
    public void MonthlyRate_lt_projects_with_number_coercion()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo(1000000m));
        Assert.That(rule.Shape, Is.EqualTo(Alis.Reactive.PlanModel.Shape.Number));
    }
    ...
    public void Status_notEqual_projects_with_constraint()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo("deleted"));
    }
    ...
    public void AdmissionDate_min_projects_with_date_coercion_and_ISO_format()
    {
        ...
        Assert.That(rule.LiteralOperand().Value, Is.EqualTo("2020-01-01"));
    ...
    public void DischargeDate_gt_projects_with_field_and_date_coercion()
    {
        ...
        Assert.That(rule.PeerOperand().FieldName, Is.EqualTo("AdmissionDate"));
        Assert.That(rule.Shape, Is.EqualTo(Alis.Reactive.PlanModel.Shape.Date));
    }
    ...
    public void All_rules_project_identically_under_WhenField_condition()
    {
        ...
        var conditionalFields = conditional.Select(f => f.FieldName).Where(f => f != "IsEmployed").OrderBy(x => x).ToList();
        Assert.That(conditionalFields, Is.EqualTo(unconditionalFields));

        // Every rule in conditional has the IsEmployed condition
        foreach (var field in conditional.Where(f => f.FieldName != "IsEmployed"))
        {
            foreach (var rule in field.Rules)
            {
                var ruleWhen = (FieldCompare)rule.WhenCondition();
        ...
        // Same rule types, constraints, fields, and shape values
        foreach (var uField in unconditional)
        {
            var cField = conditional.First(f => f.FieldName == uField.FieldName);
            ...
            for (int i = 0; i < uField.Rules.Count; i++)
            {
                ...
                cField.Rules[i].HasSameOperandAs(uField.Rules[i]);
            }
        }
    }
}
