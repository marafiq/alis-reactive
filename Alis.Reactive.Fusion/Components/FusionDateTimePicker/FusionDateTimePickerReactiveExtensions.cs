...
namespace Alis.Reactive.Fusion.Components
{
    ...
    public static class FusionDateTimePickerReactiveExtensions
    {
        ...
        public static DateTimePickerBuilder Reactive<TModel, TArgs>(
            ...
            Func<FusionDateTimePickerEvents, ReactiveEvent<TArgs>> eventSelector,
            Action<TArgs, PipelineBuilder<TModel>> pipeline)
            where TModel : class
        {
            var reactiveEvent = eventSelector(FusionDateTimePickerEvents.Instance);

            var attrs = (IDictionary<string, object>)builder.model.HtmlAttributes;
            var componentId = (string)attrs["id"];
            var scope = plan.Authoring.CreateObjectEventScope(
                componentId,
                FusionDateTimePicker.Definition,
