...
namespace Alis.Reactive.Fusion.Components
{
    ...
    public static class FusionMultiColumnComboBoxReactiveExtensions
    {
        ...
        public static MultiColumnComboBoxBuilder Reactive<TModel, TArgs>(
            ...
            Func<FusionMultiColumnComboBoxEvents, ReactiveEvent<TArgs>> eventSelector,
            Action<TArgs, PipelineBuilder<TModel>> pipeline)
            where TModel : class
        {
            var reactiveEvent = eventSelector(FusionMultiColumnComboBoxEvents.Instance);

            var attrs = (IDictionary<string, object>)builder.model.HtmlAttributes;
            var componentId = (string)attrs["id"];
            var scope = plan.Authoring.CreateObjectEventScope(
                componentId,
                FusionMultiColumnComboBox.Definition,
