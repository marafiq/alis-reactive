...
From the [Grammar Tree](../../mental-model/#the-grammar-tree) — the component API:
...
A component's initial value comes from model binding — `@Model.Country` prefills the dropdown when the page renders. `.SetValue()` changes it dynamically at runtime, in response to a trigger or event. Both are correct for their stage: model binding sets the initial state, pipeline reactions change it after.
...
**Next:** [Conditions](../conditions/) — runtime branching with When/Then/ElseIf/Else and guard composition.
