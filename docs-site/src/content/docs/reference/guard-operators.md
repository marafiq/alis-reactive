...
Use `.Value()` on a `ComponentRef` to get a typed value expression. The runtime resolves the component object and reads its bound value member:
...
Every comparison operator also accepts a `ValueExpression<TProp>` on the right-hand side, enabling comparisons between two live values:
...
Every guard terminates with `.Then()`, which starts the branch body and returns a `BranchBuilder`.
