...
```csharp
// Properties
BindingPath { get; }  // Model property path (e.g., "AdmissionDate", "Address.Street") — key in the input component onboarding catalog.
...
Converts lambda expressions like `x => x.Address.City` into runtime,
...
```csharp
// Properties
DefaultId { get; }  // Gets the default DOM element ID for this layout-owned component.
```
...
```csharp
// Properties
ValueMember { get; }  // Gets the member name on the JavaScript component object that gather and validation read.
```
...
```csharp
// Properties
Name { get; }  // Gets the plugin name used by the runtime.
```
...
Base descriptor for a declared Reactive Plan plugin operation.

```csharp
// Properties
Member { get; }  // Gets the declared plugin target name; root functions report `root`.
PluginName { get; }  // Gets the plugin registration name.
```
...
```csharp
// Properties
Member { get; }  // Gets the readable plugin property name.
PluginName { get; }  // Gets the plugin registration name.
```
...
```csharp
// Properties
Args { get; }  // Gets the typed payload placeholder used to author event-argument reads.
ObjectEvent { get; }  // Gets the component-object event name written into the Reactive Plan.
```
...
A typed, deferred array transform. Operators capture authoring intent and compile to
            plan-JSON `array-op` nodes — nothing executes on the server. Deliberately NOT
...
A scalar value produced by an array operation (count, sum, any, find, ...). It is a
            `TypedSource<T>`, so it plugs into the places that accept the base
            `TypedSource<T>` — `SetText`, `When`, and dispatch payloads — with no new
...
Continues a condition branch chain after a matching `Then` case.
...
Shape { get; }  // Gets the expected type shape. Defaults to none when not specified.
```
...
```csharp
// Properties
ItemShape { get; }  // Gets the declared element shape of the source array.
Kind { get; }  // Gets the kind. Always `"array-op"`.
Op { get; }  // Gets the operation name, such as `count` or `filter`.
Predicate { get; }  // Gets the per-element predicate, or null when the operation does not use one.
Projection { get; }  // Gets the per-element projection, or null when the operation does not use one.
Shape { get; }  // Gets the declared output shape of the operation.
Source { get; }  // Gets the value expression that produces the source array.
```
...
```csharp
// Properties
Guard { get; }  // Gets the guard that decides whether this branch case runs.
...
```csharp
// Properties
Args { get; }  // Gets the method arguments. Empty when the method takes no arguments.
...
```csharp
// Properties
ItemShape { get; }  // Gets the element type shape used by collection operators such as `contains`. `None` for non-collection comparisons.
...
Shape { get; }  // Gets the expected type shape for comparison. `None` when not specified.
```
...
```csharp
// Properties
Container { get; }  // Gets the container component ID to validate.
Kind { get; }  // Gets the kind. Always `"container"`.
```
...
```csharp
// Properties
Element { get; }  // Gets the element id resolved via getElementById.
Kind { get; }  // Gets the kind. Always `"dom"`.
```
...
```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"status"`.
Status { get; }  // Gets the HTTP status code to match.
```
...
```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"follow-up"`.
Next { get; }  // Gets the request to run after the current request succeeds.
```
...
Slot { get; }  // Gets the partial slot that receives injected HTML.
...
Invokes a method and uses the returned value.

```csharp
// Properties
Args { get; }  // Gets the method argument expressions in call order.
Kind { get; }  // Gets the kind. Always `"method"`.
```
...
```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"none"`.
```
...
```csharp
// Properties
Kind { get; }  // Gets the completion kind.
```
...
```csharp
// Properties
Completion { get; }  // Gets the completion behavior to run after all steps settle.
...
Represents a partial plan emitted by a view that can be loaded into a DOM slot.

```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"partial"`.
```
...
```csharp
// Properties
Kind { get; }  // Gets the payload contract kind.
```
...
Type { get; }  // Gets the payload typing contract.
```
...
```csharp
// Properties
ValidationJobs { get; }  // The validation jobs declared during plan construction.
```
...
DeclareObjectTarget()
DeclarePluginMethod()
DeclarePluginProperty()
DeclareProperty()
GetComponent()
RegisterInputComponents()
RegisterPlugin()
RegisterValidationJob()
RequireRegistrationById()
SetComponent()
WireComponentEvent()
```
...
Name { get; }  // Gets the plan-registered plugin name.
Type { get; }  // Gets the plugin object contract type key.
```

### PropertyValueReadAccess

Reads a property value from the source.

```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"property"`.
```
...
A value read from a live source when the Reactive Plan runtime executes.

```csharp
// Properties
Access { get; }  // Gets whether the read accesses a property or invokes a method.
From { get; }  // Gets the value source for this read, such as a component or payload scope.
Kind { get; }  // Gets the kind. Always `"read"`.
Member { get; }  // Gets the property or method name to read on the source.
Path { get; }  // Gets the nested property path. Defaults to empty for direct reads.
Shape { get; }  // Gets the expected type shape. Defaults to none when not specified.
```
...
```csharp
// Properties
Chain { get; }  // Gets the request chain: terminal or followed by another request.
Error { get; }  // Gets the error response routes.
Finally { get; }  // Gets reactions to execute after the request settles regardless of outcome.
Input { get; }  // Gets the request body strategy. Bodiless requests use `NoRequestInput`.
Method { get; }  // Gets the HTTP method (GET, POST, PUT, DELETE).
Success { get; }  // Gets the success response routes.
Url { get; }  // Gets the request URL, which may contain template placeholders like `{id}`.
Validation { get; }  // Gets the validation target used before sending this request.
WhileLoading { get; }  // Gets reactions to execute while the request is loading.
```
...
```csharp
// Properties
Kind { get; }  // Gets the validation target kind.
```
...
```csharp
// Properties
Match { get; }  // Gets the response status match that selects this route.
...
```csharp
// Properties
Kind { get; }  // Gets the response match kind.
```

### RootPlanScope

Represents a root view plan.

```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"root"`.
```

### RuntimeObjectSource

Identifies a plan object whose declared properties and methods can be evaluated at runtime.
...
```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"on-settled"`.
Reaction { get; }  // Gets the reaction to run after all branches settle.
```
...
```csharp
// Properties
Kind { get; }  // Gets the kind. Always `"terminal"`.
```

### UrlSource

Reads a value from the current page URL query string.
...
A validation source declared on an HTTP request during plan construction, awaiting
            ...
            at the end of `Render()`, when every component on the page is known. The job
            carries the values resolution needs — never a `RequestPlan` reference, so
...
Base class for the member access intent of a value read.

```csharp
// Properties
Kind { get; }  // Gets the access kind.
```
...
Typed post-render runtime behavior for `FusionBreadcrumb`.
...
```csharp
// Properties
Disabled { get; }  // Gets or sets whether the clicked item is disabled.
IconCss { get; }  // Gets or sets the clicked item's icon CSS classes.
Id { get; }  // Gets or sets the clicked item's id.
Text { get; }  // Gets or sets the clicked item's text.
Url { get; }  // Gets or sets the clicked item's URL.
```
...
```csharp
// Properties
Item { get; }  // Gets or sets the clicked item metadata from the Syncfusion event.
```
...
Typed post-render runtime behavior for `FusionBulletChart`.
...
Typed post-render runtime behavior for `FusionCarousel`.
...
```csharp
// Properties
Checked { get; }  // Gets or sets whether the checkbox is checked after the change.
```
...
Typed post-render runtime behavior for `FusionContextMenu`.
...
```csharp
// Properties
Disabled { get; }  // Gets or sets whether the selected item is disabled.
Id { get; }  // Gets or sets the selected item's Syncfusion id.
Text { get; }  // Gets or sets the selected item's display text.
```
...
```csharp
// Properties
Item { get; }  // Gets or sets the selected item metadata from the Syncfusion select event.
```
...
// Properties
IsInteracted { get; }  // Gets or sets whether the change was triggered by user interaction.
OldValue { get; }  // Gets or sets the previous selected value IDs.
Value { get; }  // Gets or sets the selected value IDs.
```
...
Typed runtime extensions for `FusionGrid` in reactive pipelines.
...
```csharp
// Properties
Value { get; }  // Gets or sets the selected string values.
```
...
Typed post-render runtime behavior for `FusionListBox`.
...
Typed post-render runtime behavior for `FusionListView`.
...
```csharp
// Properties
Cancel { get; }  // Gets or sets whether Syncfusion should cancel the selection.
Index { get; }  // Gets or sets the selected item's zero-based index when the event came from user interaction.
IsChecked { get; }  // Gets or sets the checkbox state for checkbox ListViews.
IsInteracted { get; }  // Gets or sets whether the selection came from user interaction.
Text { get; }  // Gets or sets the selected item's visible text.
```
...
Typed post-render runtime behavior for `FusionMenu`.
...
```csharp
// Properties
Index { get; }  // Gets or sets the zero-based field index that lost focus.
IsInteracted { get; }  // Gets or sets whether the blur came from user interaction.
Value { get; }  // Gets or sets the current OTP value.
```
...
```csharp
// Properties
Index { get; }  // Gets or sets the zero-based field index that received focus.
IsInteracted { get; }  // Gets or sets whether the focus came from user interaction.
Value { get; }  // Gets or sets the current OTP value.
```
...
```csharp
// Properties
Index { get; }  // Gets or sets the zero-based field index that changed.
PreviousValue { get; }  // Gets or sets the previous OTP value.
Value { get; }  // Gets or sets the current OTP value.
```
...
// Properties
IsInteracted { get; }  // Gets or sets whether the change was triggered by user interaction.
PreviousValue { get; }  // Gets or sets the previous committed OTP value.
Value { get; }  // Gets or sets the new OTP value.
```
...
```csharp
// Properties
CurrentDuration { get; }  // Gets or sets the current progress duration in milliseconds.
Percent { get; }  // Gets or sets the current progress percentage.
Step { get; }  // Gets or sets the progress step interval.
```
...
```csharp
// Properties
Value { get; }  // Gets or sets the selected radio button value from the Syncfusion change event.
```
...
// Properties
IsInteracted { get; }  // Gets or sets whether the change was triggered by user interaction.
PreviousValue { get; }  // Gets or sets the previous rating value.
Value { get; }  // Gets or sets the new rating value.
```
...
```csharp
// Properties
IsInteracted { get; }  // Gets or sets whether Syncfusion reports user interaction for this transition.
```
...
```csharp
// Properties
Action { get; }  // Gets or sets the Syncfusion action name for the change.
IsInteracted { get; }  // Gets or sets whether the change was triggered by user interaction.
PreviousValue { get; }  // Gets or sets the previous slider value.
Text { get; }  // Gets or sets the formatted slider value text.
Value { get; }  // Gets or sets the new slider value.
```
...
```csharp
// Properties
Value { get; }  // Gets or sets the current text value.
```
...
// Properties
IsInteracted { get; }  // Gets or sets whether the change was triggered by user interaction.
PreviousValue { get; }  // Gets or sets the previous committed text value.
Value { get; }  // Gets or sets the current text value.
```
...
```csharp
// Properties
PreviousValue { get; }  // Gets or sets the previously input text value.
Value { get; }  // Gets or sets the current text value.
```
...
Typed post-render runtime behavior for `FusionToolbar`.
...
Wires `NativeButton` DOM events into the Reactive Plan.
...
```csharp
// Properties
Value { get; }  // Gets or sets the checked values after the change.
```
...
```csharp
// Properties
BindingPath { get; }  // Gets the model binding path, such as `"Address.City"`.
...
```csharp
// Properties
Value { get; }  // Gets or sets the hidden input value after the change event.
```
...
Wires `NativeRadioGroup` DOM events into the Reactive Plan.
...
```csharp
// Properties
BindingPath { get; }  // Gets the model binding path, such as `"CareNotes"`.
...
```csharp
// Properties
Value { get; }  // Gets or sets the value captured from the change event.
```
