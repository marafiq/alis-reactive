...
{
    public static void Render(TagHelperOutput output, HeadingLevel level, ElementSpacing spacing, string? overline, string? cssClass)
    {
        output.TagName = $"h{(int)level}";
        var classes = HeadingCss.Classes(level, spacing, cssClass);
