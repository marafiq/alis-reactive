# Grid Judgment Calls: dataStateChange Clear Grouping Method
...
## Clear-Grouping Payload Decisions
...
| `action.requestType` | observed as `ungrouping` | accepted through shared action metadata | condition/display/request classification use case; clear grouping reports the ungrouping action lane |
...
| `action.columnName` | observed as `wing` after two setup groups | accepted through shared action metadata | source and trace show `clearGrouping()` delegates to the final `ungroupColumn`; the final column identity is useful display/request metadata |
| `action.cancel` | absent | not accepted for this row | clear grouping does not emit it in this trace; do not infer from other variants |
| `action.preventFocusOnGroup` | absent | not accepted for this row | grouping setup emits this internal UI flag, but clear grouping does not |
| `group` | absent | excluded for this row | clearing grouping omits the descriptor; do not model as an empty array |
| `groups` | absent | excluded for this row | duplicate grouping alias is not emitted by the clear event |
| `sorted` | absent | excluded for this row | setup grouping emitted sorted state, but clear grouping omits it |
...
Clear grouping combined with active sort, filter, search, lazy-loaded groups, or
UI-driven ungroup gestures may be reconsidered only through focused rows that
prove a real typed DSL use case and behavior proof.
