# Grid Judgment Calls: dataStateChange FilterBar Typing
...
| `where[].condition` | observed on complex predicate | accepted | transported as source-backed query metadata; combiner behavior requires a separate multi-predicate row |
...
| `aggregates` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | aggregation requires a focused row |
| `dataSource` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | data-source lanes are separate rows |
...
| `isLazyLoad` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | lazy-load behavior requires a focused row |
| `onDemandGroupInfo` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | on-demand grouping requires a focused row |
| `select` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | selection behavior requires a focused row |
| `sorted` | absent | not accepted for this row | sorting plus filtering requires a combination row |
| `table` | declared by `DataStateChangeEventArgs`, absent in trace | not accepted for this row | no typed server query use case in this variant |

Nested settings fields under the excluded `action.currentFilterObject` and
`action.columns[]` objects are inventoried below. They are not read as public
C# payload paths for this row.

| Settings path | Discovery status | C# DSL decision | Reason |
| --- | --- | --- | --- |
| action.currentFilterObject.actualFilterValue | observed | excluded | settings object detail; no durable typed server query use case |
| action.currentFilterObject.actualOperator | observed | excluded | settings object detail; no durable typed server query use case |
| action.currentFilterObject.field | observed | excluded | duplicates `where[].predicates[].field` |
| action.currentFilterObject.ignoreAccent | observed | excluded | duplicates `where[].predicates[].ignoreAccent` |
| action.currentFilterObject.isForeignKey | observed | excluded | no foreign-key behavior proven for this text-filter row |
| action.currentFilterObject.matchCase | observed | excluded | settings-only field; top-level query uses `ignoreCase` |
| action.currentFilterObject.operator | observed | excluded | duplicates `where[].predicates[].operator` |
| action.currentFilterObject.predicate | observed | excluded | settings-only field; data-state query uses `where[].condition` |
| action.currentFilterObject.predicates | observed | excluded | settings-only array; data-state query uses `where[].predicates[]` |
| action.currentFilterObject.uid | observed | excluded | browser-generated column uid, not durable typed DSL |
| action.currentFilterObject.value | observed | excluded | duplicates `where[].predicates[].value` |
| action.columns[].actualFilterValue | observed | excluded | settings object detail; no durable typed server query use case |
| action.columns[].actualOperator | observed | excluded | settings object detail; no durable typed server query use case |
| action.columns[].field | observed | excluded | duplicates `where[].predicates[].field` |
| action.columns[].ignoreAccent | observed | excluded | duplicates `where[].predicates[].ignoreAccent` |
| action.columns[].isForeignKey | observed | excluded | no foreign-key behavior proven for this text-filter row |
| action.columns[].matchCase | observed | excluded | settings-only field; top-level query uses `ignoreCase` |
| action.columns[].operator | observed | excluded | duplicates `where[].predicates[].operator` |
| action.columns[].predicate | observed | excluded | settings-only field; data-state query uses `where[].condition` |
| action.columns[].predicates | observed | excluded | settings-only array; data-state query uses `where[].predicates[]` |
| action.columns[].uid | observed | excluded | browser-generated column uid, not durable typed DSL |
| action.columns[].value | observed | excluded | duplicates `where[].predicates[].value` |
