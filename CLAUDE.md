...
- Event args properties (Fusion): `{ get; private set; }` — read-only phantom types
...
**XML docs voice for event args:** "Gets the..." (not "Gets or sets") — these are read-only
phantom types. No "Syncfusion" prefix in docs — use the framework class name directly
