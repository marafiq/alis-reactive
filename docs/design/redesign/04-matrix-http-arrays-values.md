...
| **Literal — arbitrary value** (enum, Guid, object) | `ArgValue(value)` / `Static(k, value)` | Value → `LiteralFromValue(value)` → `Shape.FromValue` (enum/Guid → `string`; collection → `array<…>`; else `any`) | `{ "kind":"literal", "value":<json>, "shape":<inferred> }`. STJ serializes the value at Render. | `any` when the type is unclassifiable — never a guessed scalar. |
