...
| payload scope `dispatch` | factory prepared (`Source.cs:73`), zero callers | Same staged pattern presumed; resolves to `values.event` at runtime. Confirm with owner before any narrowing. |
| payload scope `local` | factory prepared (`Source.cs:75`), zero callers; no live runtime creator either | Same staged pattern presumed; confirm intent with owner before any narrowing. |
