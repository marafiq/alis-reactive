...
- Audit means improving skill accuracy. Every audit defect, invalid probe
  assumption, missing field, or false proof claim must update or confirm the
  reusable pattern map under
...
- Proof must target realistic behavior of the onboarded capability. Do not
  shape tests around helper syntax, plan JSON, or a narrow fake gesture that
  passes while missing the real Syncfusion user/component behavior.
- Remote data is a primary behavior lane for components that bind, query,
  refresh, or replace data after render. For Grid-like components, remote or
  custom-binding proof must cover realistic data-state/user workflows, server
  shaped `{ result, count }` payloads, typed response-body `SetDataSource`,
  visible row refresh, and any relevant `Data`, `Refresh`, DataManager,
  adaptor, or nested data-source behavior. Static local-array proof is not
  enough when the component has meaningful remote/custom-binding behavior.
...
- All artifacts must hold shape end to end. A defect means at least one artifact
  ...
  The fix must update the skill pattern map so future component rows cannot
...
For complex components, prove one API row end to end before starting the next
row. Finish events first, one event and one trigger variant at a time, then
properties, then methods. A same-named event fired by different gestures is
multiple rows until raw traces prove the payload shapes are equivalent.

For event rows, the payload contract is required evidence. The row must prove
exact keys, missing/optional keys, nested object shapes, arrays through the
proper array primitive or whole typed array sources, writable payload fields,
payload methods, and typed C# event arg names before primitive mapping.

The typed API coverage matrix must include property-level payload rows. A green
payload class row is invalid if any public member on that payload class lacks
its own raw trace, primitive mapping, vertical slice row, and typed DSL
Playwright proof.

The typed API coverage matrix must also uniquely identify overloads and
same-name lifecycle helpers. `SetDataSource` overloads, payload mutation methods
such as `Cancel`, and same-name component methods/events cannot share one row
unless their signatures and behavior are proven equivalent.

Open behavior lanes must be matrix rows, not prose-only warnings. If discovery,
review, or audit identifies a required lane such as remote data,
DataManager/adaptor, nested data-source paths, response-shape variants,
overload families, or event-trigger variants, the generated coverage matrix must
emit a stable fail-closed row for that lane until raw trace, primitive mapping,
vertical slice, and realistic typed DSL Playwright proof close it.

Generated supplemental rows must be artifact-derived. For event variants, the
judgment-call artifact is the accepted/excluded decision source. Proof status is
separate: an accepted judgment row without current typed DSL behavior proof must
remain an unproven matrix row.
...
Exclusion rows are fail-closed. A judgment call, missing request-body field, or
removed C# member is supporting evidence only; it does not prove the row. The
artifact must include explicit source-backed or behavior-backed exclusion proof
for the exact public typed contract member before the matrix can mark that row
proven.
