...
- Precise plan vocabulary. Use "browser" only for DOM/window/network APIs,
  user-agent behavior, Playwright-visible outcomes, or generated
  `BrowserObject*` contract terms that still exist. Use "Reactive Plan" for
  authored or serialized plan contracts, "Active Plan" for runtime-composed plan
  state, and "plan-registered component" or "runtime boundary" for narrower
...
- Preserve stale vocabulary that no longer matches the DSL graph or domain
  terms.
- Widen "browser" into a synonym for the Reactive Plan, Active Plan, component
  contract, or framework runtime.
