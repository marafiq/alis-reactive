...
- focused HTTP / merge / enrichment rerun after the same fixes:
  **6 files, 153 tests passed**
- focused realtime end-to-end rerun after pooled-subscriber fixes:
  **2 files, 2 tests passed**
- authoritative schema-center TS sweep:
  **21 files, 409 tests passed**
- core reactive C# sweep: **80 tests passed**
- focused C# lowering sweep: **32 tests passed**, including **4 new lowering-truth tests**
- focused C# request/trigger/lowering rerun after the same fixes:
  **47 tests passed**
- widened C# builder/trigger/validation seam sweep:
  **77 tests passed**
...
| request gather from response payload | [when-chaining-http-requests.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-chaining-http-requests.test.ts) | [ResponseBody.cs](../../Alis.Reactive/ResponseBody.cs) proves typed response reads inside `OnSuccess`; current public DSL does not yet prove response-based `chained.gather` lowering directly | `Request.response.chained` must keep response scope readable when the schema/runtime support response-based chained gather | no | no | runtime-proven; DSL exposure pending |
...
| partial component-event reload lifecycle | [when-removing-event-listeners.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-removing-event-listeners.test.ts), [when-enriching-after-merge.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-enriching-after-merge.test.ts), [when-managing-pooled-realtime-partials.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-managing-pooled-realtime-partials.test.ts) | [merge-plan.ts](../../Alis.Reactive.SandboxApp/Scripts/lifecycle/merge-plan.ts), [server-push.ts](../../Alis.Reactive.SandboxApp/Scripts/execution/server-push.ts), [signalr.ts](../../Alis.Reactive.SandboxApp/Scripts/execution/signalr.ts) are the operative runtime seams | component event and realtime trigger subscribers must be self-sufficient at wire time and teardown must remain source-scoped | yes | no | proven |
...
| SSE | [when-triggering-on-server-push.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-triggering-on-server-push.test.ts), [when-proving-server-push-pipeline-semantics.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-proving-server-push-pipeline-semantics.test.ts), [when-managing-pooled-realtime-partials.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-managing-pooled-realtime-partials.test.ts) | [WhenTriggeringOnServerPush.cs](../../tests/Alis.Reactive.UnitTests/Triggers/WhenTriggeringOnServerPush.cs), [server-push.ts](../../Alis.Reactive.SandboxApp/Scripts/execution/server-push.ts) | explicit SSE trigger with carried host message payload and subscriber-scoped shared transport teardown | yes | no | proven |
| SignalR | [when-triggering-on-signalr.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-triggering-on-signalr.test.ts), [when-proving-signalr-pipeline-semantics.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-proving-signalr-pipeline-semantics.test.ts), [when-managing-pooled-realtime-partials.test.ts](../../Alis.Reactive.SandboxApp/Scripts/__tests__/when-managing-pooled-realtime-partials.test.ts) | [WhenTriggeringOnSignalR.cs](../../tests/Alis.Reactive.UnitTests/Triggers/WhenTriggeringOnSignalR.cs), [signalr.ts](../../Alis.Reactive.SandboxApp/Scripts/execution/signalr.ts) | explicit SignalR trigger with carried host callback payload and subscriber-scoped shared transport teardown | yes | no | proven |
...
### 3. There must be one value-access algebra
...
The final schema should therefore use one shared value/access algebra, with
scoped subsets only where the domain intentionally narrows what is legal
(for example validation purity or trigger-payload build roots). What it should
not do is invent unrelated request-value, command-value, and validation-value
DTO families.
...
Every required feature/workflow in the issue #86 list is anchored by at least
one real code seam and at least one fresh executable proof surface in this
pass. Where the matrix still says `schema-fit proven; end-state lowering
pending` or `runtime-proven; DSL exposure pending`, that remaining gap is
intentional and explicit rather than hidden by the conclusion.
