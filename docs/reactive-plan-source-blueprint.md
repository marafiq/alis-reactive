...
| vendor gather include | `g.Include<Component,TModel>(...)` or native shorthand | component value assignment + input contract | component property read | read component value for request |
...
| direct client validation `registry.For(...).Field(...)` | field rule projection | `ClientValidationField` with rules | validation rules in plan | browser validation runtime |
