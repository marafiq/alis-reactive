...
| `p.Into(elementId)` | inject success body into host | inject reaction with success payload body | `reaction.kind="inject"`, `target.slot` | write HTML, discover plans, load slot |
...
| T17 plugin read and command | `p.Plugin<int>(fn).Arg(p.FromUrl<int>("id"))` and `p.Plugin(cmd).Arg("x").Fire()` | plugin contract with typed args/return; read value expression and call reaction | plugin object contract + value/call entries | runtime resolves plugin, args, return/call | plugin runtime tests + ArrayManager typed plugin Playwright |
