...
| Supplemental Gather Fields | Explicit request-input contract for supplemental fields. `none` means no supplemental fields; `declared` carries the static/event `ValueProducer`, so missing JSON is never behavior. |
