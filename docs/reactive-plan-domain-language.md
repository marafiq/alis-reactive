...
| Partial Listener Lifetime | Abort-signal lifetime owned by one slot load and shared by every contribution in that load. Component-event behavior and live validation listeners attach to it so unload can stop browser callbacks deterministically. |
