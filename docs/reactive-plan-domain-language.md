...
- **Field Comparison Operand**: the right side of a field comparison: absent,
  literal, or array.
- **Omitted Client Rule**: a validator rule that still runs through the
  original validator but was not proven deterministic for browser execution and
  therefore does not enter the Reactive Plan.
...
replace slot:
  ...
  create one AbortController for the Slot Load
...
| Validation adapter projection | Client validation fields | Bound validation rules and activations | Evaluate browser rules and display errors | Server rules always remain authoritative; client projection is deterministic subset |
...
| Initial DOM with multiple same-plan scripts | Root and partial scripts can already be present at first boot | Initial composition and browser slot load must share contribution policy |
...
| `runtime/lifecycle` | Plan lifetime | Boot composition, partial slot replacement, ownership, slot load aborting | Component member execution or validation rule semantics |
...
| Shared contribution assembler | Initial boot composition and browser slot load should use one contribution language so merge rules cannot drift. | Boot composition and slot load now share component contribution vocabulary; boot keeps only the extra validation-container replacement rule because initial documents have no unload lifetime. |
...
| Deferred validation field binding | Deferred validation fields bind shape knowledge after projection and should stay outside FluentValidation-specific vocabulary. | `ClientValidationField` now carries a projected field reference; render-time binding no longer reflects over the model to infer shape. |
...
| --- | --- | --- | --- | --- |
| Accepted | Browser object contract fragments merge | Last-writer-wins type replacement | One browser object can be declared by reads, writes, events, gather, validation, plugins, root views, and partials. Replacement loses intent. | `BrowserObjectContract.Declare`, `mergeObjectContracts`, `WhenDeclaringJsObjectContracts`, `merge-plan.test.ts` |
| Accepted | Shape-compatible object contract fragments | Runtime JSON shape equality | C# `ShapeContractCompatibility` and runtime partial merge must agree: `any` refines to concrete, nullable/array/object contracts merge recursively, and `none` stays a conflict. | `ShapeContractCompatibility`, `object-contract-fragment.ts`, browser object contract lifecycle tests |
| Accepted | Input value contract enrichment | Replacing a component type with a read-only registered-input type | Registered input metadata is gather/validation read intent, not exclusive ownership of the browser object contract. A behavior may also write the same member. | `BrowserObjectContracts.EnsureInputValueContract`, `WhenDeclaringInputValueContracts`, AdmissionAssessment Playwright slice |
| Accepted | Initial boot composition uses contribution language | Boot-time `Object.assign` over types/components | Plan scripts present in first DOM can include root and partial contributions in any order. Boot composes generated component intents in DOM order while preserving owned input definitions from reference-only entries. | `BootPlanAssembly.accept`, `composeInitialComponentIntoPlan`, initial-composition tests, AdmissionAssessment `CareUnit` |
| Accepted | Initial component composition | Treating duplicate first-DOM owned inputs as dynamic partial ownership collisions | MVC can render multiple initial plan scripts that mention the same already-mounted input. Boot has no unload lifetime, so generated duplicate definitions are folded into the root-scoped boot plan instead of becoming runtime collision handling. | `composeInitialComponentIntoPlan`, AdmissionAssessment Playwright slice |
...
| Accepted | Client validation fields | Extracting "server rules" | FluentValidation remains server-authoritative; only deterministic browser rules enter the plan. Unsupported rules are omitted from the browser projection; the server validator remains authoritative. | `IClientValidationProjectionSource.ProjectClientRules`, `ClientValidationField`, FluentValidator unit tests |
| Accepted | Explicit peer validation projection | Reconstructing peer fields from FluentValidation `MemberInfo` | Peer-field browser validation is deterministic only when the developer declares the client projection with typed `ProjectToClient(...)`; the server FluentValidation rule still runs normally. | `ProjectToClient`, `PeerComparisonRequiresExplicitProjection`, FluentValidator unit tests |
...
| Accepted | Explicit partial slot unload | Treating partial load as append-only | Server partials can load and unload. Gather, validation, behavior listeners, type fragments, and owned components must follow slot lifetime. | `AppliedBrowserPlans.unloadPartialSlot`, partial lifecycle tests |
...
| Accepted | Explicit component contribution intent | Runtime-only inference from empty binding/container | Component entries declare whether they are an object target, owned definition, validation container, or layout object contribution. Runtime uses that intent for deterministic join/materialize/extend/unload behavior; shape correctness belongs in the generated C# plan. | `ComponentContributionIntent`, generated `plan.ts`, merge-plan intent tests |
| Accepted | Shared component contribution language | Boot-only component checks beside partial merge checks | Initial DOM scripts and dynamic partial loads are both document contributions. They share vocabulary for references, owned definitions, validation extensions, and layout objects without turning the runtime into a bad-plan validator. | `composeInitialComponentIntoPlan`, `mergeComponentIntoPlan`, `AppliedBrowserPlans.loadPartialSlot`, initial-composition tests |
...
| Accepted | Deferred validation field binding | Treating deferred validation as FluentValidation projection detail | Deferred binding resolves projection-declared shape and component id after projection, so it belongs to validation plan binding language. | `ClientValidationField`, validation partial tests |
...
question is: **is the incoming generated plan defining a browser object,
referencing an existing page-owned object, or extending a validation
container?** These are normal domain actions. The runtime tracks their
consequences for execution and unload; it does not police impossible generated
plan shapes as normal control flow.

| Classification | Owner | Merge Rule | Unload Rule |
| --- | --- | --- | --- |
| Owned Component Definition | The source that renders/introduces the component object | Materialize the generated definition and record the contributing root or slot as owner | Remove component, type ownership contribution, behavior listeners, and validation/gather reachability owned by the slot |
| Object Target Contribution | The source needs a deterministic browser object handle. If the component key is unowned, materialize it; if an existing object has the same id/vendor/type, the contribution joins it. | Join or materialize from the generated component intent; no separate collision policy | Remove the component only when the source owns it; otherwise release only the source's type fragment/listeners |
...
| Root-Owned Validation Extension | Root page owns the validation container; partial contributes fields/rules | Append validation rules for new validated components while preserving root rules for existing components | Remove only the exact rule contribution |
...
| Initial Component Composition | Boot-time merge for component entries already present in the first DOM. It preserves owned input definitions when later documents only reference the same runtime object, merges validation-container rules by validated component key, and emits one root-scoped runtime plan. |
...
| Object Contract Fragment Contribution | Source-owned object contract fragment. Root fragments and partial-slot fragments are stored separately, then materialized into the active `BrowserObjectContract`; unloading a partial releases only that source's fragment and recomputes the remaining contract. |
| Object Contract Shape Compatibility | Merge policy for a property, method argument, or method return shape declared by multiple fragments. `any` can be refined, arrays and object fields merge recursively, nullable wrappers preserve nullability, and `none` is never a wildcard. |
...
| FluentValidation Projection Draft | Source-specific accumulator used while walking a FluentValidation validator. It owns fields, projected browser rules, peer-field discovery, and shape evidence before client validation fields are returned. |
| Rule Component Mapping | Anti-corruption object for one FluentValidation rule component after target field and activation are known. It carries the validator component, target field, display name, rule message, and rule condition so rule projection does not read FluentValidation metadata from scattered call sites. |
| Range Endpoint Values | Projection object for FluentValidation range endpoints. Complete endpoints with matching shapes produce validation range bounds with serialized date literals; missing or mismatched endpoints are omitted from the browser projection rather than becoming an invented browser rule. |
| Comparison Rule Target | Projection target for FluentValidation comparison rules. It classifies fixed literals, same-object peer fields, unsupported peer shapes, and cross-object peer comparisons before emitting a deterministic browser rule. |
...
| Client Validation Projection Source | Integration contract that returns deterministic `ClientValidationField` entries for a validation source type. The validation container comes from the `ValidationJob`; projection does not carry it again. |
| Client Validation Projection Registry | Core-owned `IClientValidationProjectionSource` for directly authored deterministic browser rules. It maps a validation source type to client validation fields and keeps FluentValidation as one producer, not the projection model itself. |
| Client Validation Field Token | Opaque typed field reference built from a model expression. It carries the validation field path and shape internally so public projection authoring does not pass loose field-name strings. |
| Client Validation Field | Immutable projected validation field whose rules can be represented deterministically in the Reactive Plan and executed by the browser runtime. Builders and drafts may accumulate rules, but projection output is read-only. |
| Client Validation Field Shape Source | Field-shape ownership for deferred validation binding. Registry-authored fields carry projected shapes from typed field tokens; adapter-authored fields can still ask the model binding path for shape until that adapter supplies equivalent field metadata. |
...
| Omitted Client Rule | Validation rule omitted from the browser projection because the adapter cannot prove an equivalent deterministic browser rule, such as FluentValidation conditions without a complete client guard, cross-object peer comparisons, missing range metadata, or unsupported validator shapes. |
| Validation Rule Target | Compile-time validation rule target before render-time component binding. `none`, fixed literal/range, and peer field are separate cases so literal null, no target, and peer reads are not confused. |
| Validation Rule Condition | Validation projection activation before render-time binding. `always` means unconditional; `when` carries the symbolic `FieldCondition` tree that will resolve against registered component values. |
| Public Validation Rule Operand | Consumer/test-facing validation operand exposed by `ValidationRule.Operand`: `NoValidationRuleOperand`, `LiteralValidationRuleOperand`, `RangeValidationRuleOperand`, or `PeerFieldValidationRuleOperand`. Absence is never inferred from `null`; literal `null` is a present literal operand. |
| Public Validation Rule Activation | Consumer/test-facing validation activation exposed by `ValidationRule.Activation`: `AlwaysValidationRuleActivation` or `ConditionalValidationRuleActivation`. Tests assert the activation case directly instead of treating a missing condition as behavior. |
...
| Validation Resolution Scope | Render-time context that owns all bindings from projected validation rules to component value contracts. It is created from the registered input catalog and root model type, then binds rules, peer fields, and field conditions through the same field catalog. |
...
| Unmounted Validation Field | Validation target whose component is absent from the currently composed plan or DOM because a partial/SSR branch did not render it. The runtime asks whether every rule is inactive for that unmounted target; an unmounted activation dependency makes that unmounted target inactive, while the same missing dependency still fails for a mounted validation field. |
...
| Runtime Validation Activation | Browser execution object for validation activation. Mounted fields evaluate activation strictly; unmounted fields use the activation only to decide whether the missing target has an active browser rule. |
| Runtime Validation Peer Operand | Browser execution object for `otherValue`. `none` means the rule has no peer operand; `value` resolves the declared producer before rule evaluation, so a missing peer component remains a component resolution failure. |
...
| Object Target Contribution | `object-target` contribution used by property reads, property writes, method calls, injection hosts, and explicit-id component references. It can materialize an unowned component key, or join an existing root-owned component when id/vendor/type match. |
...
| Generated Plan Trust Boundary | Runtime rule that component contribution shape is owned by the C# plan model and generated contract. Lifecycle code tracks execution/unload consequences of the generated plan instead of validating alternate impossible shapes. |
| Reference-Only Component Contribution | Shared runtime lifecycle outcome for `object-target` and `layout-object` entries that join an existing runtime object with the same id/vendor/type. Boot composition and dynamic partial merge use the same join language. |
...
| Root-Owned Validation Extension | Partial contribution that adds rules to a root validation container while preserving root rules for already-known validated components. |
| Existing Component Reference | Runtime outcome for an object-target contribution that joins a component object already owned by the root page or an earlier boot contribution with the same id, vendor, and type key. This is how root hosts are used from partial behavior without being deleted on unload. |
...
| Component Event Contract | Declared event member of a component object contract. It maps the DSL event name to the concrete browser/vendor channel and payload contract. Runtime event triggers must execute this contract; they do not infer a channel from the event name when the contract is absent. |
