...
| FluentValidation Projection Draft | Source-specific accumulator used while walking a FluentValidation validator. It owns fields, projected rule intents, peer-field discovery, skipped client projections, and shape evidence before the generic `Client Validation Projection` is emitted. |
...
| Range Endpoint Values | Projection object for FluentValidation range endpoints. Complete endpoints with matching shapes produce validation range bounds with serialized date literals; missing or mismatched endpoints produce a skipped client projection rather than an invented browser rule. |
...
| Client Validation Projection Request | Integration contract given to a render-time validation projection source. It carries the validation source type and `Validation Container Id`, so projection is anchored to the validation boundary instead of a loose form string. |
...
| Client Validation Projection Registry | Core-owned `IClientValidationProjectionSource` for directly authored deterministic browser rules. It maps a validation source type to a model projection and keeps FluentValidation as one producer, not the projection model itself. |
