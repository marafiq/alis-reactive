...
| Accepted | Deferred validation model binding | Treating deferred validation as FluentValidation projection detail | Deferred binding resolves projection-declared or model-derived shape and component id after projection, so it belongs to validation plan binding language. | `ClientValidationFieldShapeSource`, validation partial tests |
